<template>
  <div class="container mx-auto py-8 px-4">
    ...
    <header class="text-center mb-8">
      <h1 class="text-4xl font-bold text-gradient mb-3">Convertidor de Variables de Entorno y Base64</h1>
      <p class="text-gray-600 text-lg">Codifica y decodifica fácilmente tus archivos .env</p>
    </header>
    
    <Card class="card mb-8 shadow-lg">
      <template #content>
        <TabView class="custom-tabview">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div class="flex flex-col">
                <div class="flex justify-between items-center mb-3">
                  <h2 class="text-xl font-medium">Archivo .env Original</h2>
                  <div class="text-xs text-gray-500" v-if="envStats.lines > 0">
                    {{ envStats.lines }} líneas | {{ envStats.vars }} variables
                  </div>
                </div>
                
                <div 
                  class="dropzone mb-3"
                  :class="{'active': isDraggingEncode}"
                ...
                <Textarea 
                  v-model="envInput" 
                  class="textarea flex-grow custom-textarea"
                  :class="{'has-content': envInput.trim()}"
                  rows="15" 
                ...
                <div class="text-right mt-1 text-xs text-gray-500">
                  <i class="pi pi-info-circle"></i> Presiona Ctrl+Enter para convertir
                </div>
              </div>
              
              <div class="flex flex-col">
                <div class="flex justify-between items-center mb-3">
                  <h2 class="text-xl font-medium">Resultado (Base64)</h2>
                  <div class="text-xs text-gray-500" v-if="base64Output">
                    {{ base64Output.split('\n').filter(line => line.trim()).length }} líneas
                  </div>
                </div>
                
                <div class="action-bar mb-3 ">
                  <span class="p-buttonset flex items-center justify-between">
                    <Button 
                      icon="pi pi-refresh" 
                      label="Convertir" 
                ...
                <Textarea 
                  ...
                  class="textarea flex-grow custom-textarea result-area"
                  :class="{'has-content': base64Output.trim()}"
                  rows="15" 
                  readonly 
                  placeholder="El resultado aparecerá aquí..."
                />
                <div class="result-actions mt-2 flex justify-end gap-2" v-if="base64Output.trim()">
                  <Badge value="Listo para usar" severity="success" />
                </div>
              </div>
            </div>
          </TabPanel>
          ...
          <TabPanel header="Decodificar Base64 a .env">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div class="flex flex-col">
                <div class="flex justify-between items-center mb-3">
                  <h2 class="text-xl font-medium">Valores Base64</h2>
                  <div class="text-xs text-gray-500" v-if="base64Input.trim()">
                    {{ base64Input.split('\n').filter(line => line.trim()).length }} líneas
                  </div>
                </div>
                ...
                <Textarea 
                  v-model="base64Input" 
                  class="textarea flex-grow custom-textarea"
                  :class="{'has-content': base64Input.trim()}"
                  rows="15" 
              ...
              <div class="flex flex-col">
                <div class="flex justify-between items-center mb-3">
                  <h2 class="text-xl font-medium">Resultado (.env)</h2>
                  <div class="text-xs text-gray-500" v-if="envStats.decodedVars > 0">
                    {{ envOutput.split('\n').filter(line => line.trim()).length }} líneas | {{ envStats.decodedVars }} variables
                  </div>
                </div>
                ...
                <div class="action-bar mb-3">
                  <span class="p-buttonset flex items-center justify-between">
                    <Button 
                      icon="pi pi-refresh" 
                      label="Convertir" 
                      @click="decodeBase64ToEnv" 
                      :disabled="!base64Input.trim()"
                      :loading="isProcessingDecode"
                      class="p-button-raised"
                      v-tooltip.bottom="'Convertir a formato .env'"
                    />
                    <Button 
                      icon="pi pi-copy" 
                      label="Copiar" 
                ...
                <Textarea 
                  v-model="envOutput" 
                  class="textarea flex-grow custom-textarea result-area"
                  :class="{'has-content': envOutput.trim()}"
                  rows="15" 
                  readonly 
                  placeholder="El resultado aparecerá aquí..."
                />
                <div class="result-actions mt-2 flex justify-end gap-2" v-if="envOutput.trim()">
                  <Badge value="Listo para usar" severity="success" />
                </div>
              </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="feature-block">
            ...
            <ol class="list-decimal ml-5 space-y-3">
              <li>Pega el contenido de tu archivo .env en el campo de entrada <span class="badge">o</span> arrastra y suelta tu archivo</li>
              <li>Haz clic en <Button label="Convertir" size="small" icon="pi pi-refresh" class="inline-button" disabled /> para generar el formato KEY:base64value</li>
              <li>Usa el botón <Button label="Copiar" size="small" icon="pi pi-copy" class="p-button-secondary inline-button" disabled /> para copiar el resultado al portapapeles</li>
              <li>También puedes descargar el resultado como un archivo de texto</li>
            </ol>
            <div class="mt-3 bg-gray-50 p-3 rounded-lg border-l-4 border-blue-500">
              <div class="text-sm"><i class="pi pi-bolt text-blue-500"></i> <strong>Atajo:</strong> Presiona <kbd>Ctrl</kbd>+<kbd>Enter</kbd> para convertir rápidamente.</div>
            </div>
          </div>
          <div class="feature-block">
            <h3 class="text-lg font-semibold mb-3 flex items-center">
            ...
            <ol class="list-decimal ml-5 space-y-3">
              <li>Pega el contenido en formato KEY:base64value <span class="badge">o</span> arrastra y suelta tu archivo</li>
              <li>Haz clic en <Button label="Convertir" size="small" icon="pi pi-refresh" class="inline-button" disabled /> para generar el archivo .env</li>
              <li>Usa <Button label="Copiar" size="small" icon="pi pi-copy" class="p-button-secondary inline-button" disabled /> para copiar el resultado</li>
              <li>Descarga el archivo .env completo con <Button label="Descargar" size="small" icon="pi pi-download" class="p-button-success inline-button" disabled /></li>
            </ol>
            <div class="mt-3 bg-gray-50 p-3 rounded-lg border-l-4 border-green-500">
              <div class="text-sm"><i class="pi pi-check-circle text-green-500"></i> <strong>Tip:</strong> Las variables que contengan espacios se guardarán automáticamente entre comillas.</div>
            </div>
          </div>
        </div>
      </template>
    ...
    <footer class="mt-10 text-center">
      <div class="p-3 bg-gray-50 rounded-lg inline-block">
      <!-- desarrolado por alarcon https://github.com/alfonso2254 y poner el nombre del repositorio  -->
        <a href="https://github.com/alfonso2254/env-base64-converter" target="_blank" class="text-blue-500 hover:underline">Desarrollado por alarcon</a>
  ...
  try {
    if (!base64Input.value.trim()) {
      toast.add({
        severity: 'warn',
        summary: 'Campo vacío',
        detail: 'Por favor introduce el contenido a decodificar',
        life: 3000
      });
      return;
    }
    ...
    setTimeout(() => {
      ...
      toast.add({
        severity: 'success',
        summary: 'Decodificación completada',
        detail: 'Variables decodificadas correctamente',
        life: 3000
      });
    }, 300);
  } catch (error) {
...
// Observar cambios en envInput para actualizar estadísticas
onMounted(() => {
  const observer = new MutationObserver(updateEnvStats);
  if (envInput.value) {
    updateEnvStats();
  }
  
  // Añadir listeners para atajos de teclado globales
